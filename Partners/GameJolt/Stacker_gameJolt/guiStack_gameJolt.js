...
playGame.prototype = {
    ...
    tick: function () {
        ...
        if (timeLeft == 9) {                      
            ...
            this.timeText.size = this.timeText.size + 10;
    ...
    removeCrate: function () {
        if (this.crateGroup.children.length > 0) {
            ...
            if (scoreCheck >= levelCheck) {
                ...
                ///gameJolt High Score Leaderboard
                try{
                    GJAPI.ScoreAdd(622587, this.score, this.score +" Points", "High Score", function(pResponse) {return;});
                }catch(err){console.log(err)}
                
                LEVEL++;
                localStorage.setItem("stackerLevel", LEVEL);
                  ///gameJolt Level Leaderboard
                  try{
                    GJAPI.ScoreAdd(622598, LEVEL, "Level " + LEVEL, "Max Level", function(pResponse) {return;});
                }catch(err){console.log(err)}
                ...
                switch(LEVEL){
                        ...
                        case 2:
                            gameJoltTrophyAwardTry(141241,0)
                            break;
                        case 5:
                            gameJoltTrophyAwardTry(141242,1)
                            break;
                        case 10:
                            gameJoltTrophyAwardTry(141243,2)
                            break;
                        case 20:
                            gameJoltTrophyAwardTry(141244,3)
                            break;
                        default:
                            return;
                }
...
function gameJoltTrophyAwardTry(numTried, numAwarded){
    try{
        if(jsonArr[numAwarded].achieved == "false"){
            GJAPI.TrophyAchieve(numTried, function(pResponse) {console.log(pResponse.response);}); console.log("gameJolt trophy - " + jsonArr[numAwarded].title + "- awarded to user")
        }else{console.log("gameJolt trophy " + jsonArr[numAwarded].title +" detect to already be achieved with value " + jsonArr[numAwarded].achieved )}
    }catch(err){return;}
}
window.setTimeout(() => {
    gameJoltTrophyAwardTry(141241, 0);
